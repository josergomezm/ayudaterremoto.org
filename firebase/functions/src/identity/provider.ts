...
export interface IdentityRecord {
  ...
  dni: string;
  fullname: string;
  // CNE returns the registered VOTING CENTER, not a home address. Use only as a
  // coarse locality hint — never as a report location.
  state: string;
  municipality: string;
}
...
const SEED: Record<string, IdentityRecord> = {
...
function synthName(key: string): string {
  const h = hash(key);
...
export const stubProvider: IdentityProvider = {
  async lookup(nac, dni) {
    if (!/^\d{6,9}$/.test(dni)) return null;        // CNE 302/303: non-numeric / invalid
 ...
 */
export function buildNameOptions(realName: string, seed: string, count = 20): string[] {
  const options = new Set<string>([realName]);
  let i = 0;
...
export const pnpProvider: IdentityProvider = {
  async lookup(nac, dni) {
    ...
    try {
      ...
      if (!nombresMatch || !primerApellidoMatch) return null;
