const es = {
  ...
  how: [
    ...
    'Mapa y triaje: los coordinadores ven los incidentes en un mapa con colores según la gravedad. Los sitios marcados como evacuados salen de la cola de búsqueda activa.',
    'Confianza: la identidad es una verificación blanda con la Cédula; la confianza real viene de una cadena de avales. Los organizadores emiten códigos de un solo uso que convierten a voluntarios en Coordinadores, y todo queda registrado para rendición de cuentas.',
    'Alertas y guías: los organizadores pueden transmitir alertas oficiales, y las guías de seguridad sin conexión explican qué hacer durante y después de un sismo, ante fugas de gas, primeros auxilios y cómo evaluar edificios dañados.',
  ],
...
const en: typeof es = {
  ...
  how: [
    ...
    'Map & triage: coordinators see incidents on a map color-coded by severity. Sites marked evacuated leave the active search queue.',
    'Trust: identity is a soft check via the Cédula; real trust comes from a vouch chain. Organizers issue single-use codes that turn volunteers into Coordinators, all logged for accountability.',
    'Alerts & guides: Organizers can broadcast official alerts, and the offline safety guides explain what to do during and after a quake, for gas leaks, first aid, and how to judge damaged buildings.',
  ],
